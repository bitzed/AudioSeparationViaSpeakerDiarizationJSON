 ...
 * @param {string|{base64:string}} source  a publicly reachable audio URL, or
 *        an object carrying base64-encoded file bytes (for ephemeral uploads
 *        that have no public URL).
 ...
 */
async function transcribe(source, configOverride = {}) {
  const token = getAccessToken();
  const file = typeof source === "string" ? source : source.base64;
  const body = {
    config: {
      language: "ja-JP",
      timestamps: true,
